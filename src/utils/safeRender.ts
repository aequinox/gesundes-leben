...
export async function safelyRender<T>(
  ...
  for (let attempt = 0; attempt <= fullConfig.maxRetries; attempt++) {
    try {
      ...
      if (fullConfig.logErrors) {
        const attemptMsg = attempt < fullConfig.maxRetries 
  ...
  if (fullConfig.showDevErrors && import.meta.env.DEV) {
    throw isAppError(lastError) ? lastError : createError(
      errorMessage || 'Safe render operation failed',
      { 
...
export function safelyExecute<T>(
  ...
  try {
    ...
    if (fullConfig.showDevErrors && import.meta.env.DEV) {
      throw isAppError(error) ? error : createError(
        errorMessage || 'Safe execute operation failed',
        { 
          componentName: fullConfig.componentName,
          originalError: error
        }
