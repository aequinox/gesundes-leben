// Node.js compatible version of post.js for standalone script usage
import satori from "satori";

// Import only standalone-compatible utilities
import {
  OG_COLORS,
  OG_EFFECTS,
  OG_IMAGE_DIMENSIONS,
...
// Simple author name formatter (no astro:content dependency)
...
// Simplified font loader for standalone usage
...
export default async (post, config) => {
  const SITE = config.SITE;
  const title = post.data.title;
  const author = formatAuthorName(post.data.author);
  // eslint-disable-next-line no-console
  console.log("Generating OG image for post:", title, "by", author);

  // Hardcode translations for standalone usage
  const authorLabel = "Autor";
