...
const TOPIC_CLUSTERS = {
  'mentale-gesundheit': {
    ...
    keywords: ['depression', 'angst', 'stress', 'mental', 'psyche', 'gefühle', 'emotionen', 'resilienz', 'achtsamkeit', 'meditation'],
    pillarPost: '2024-10-18-17-mentale-blockaden-die-erfolg-verhindern',
    supportingCategories: ['Mentale Gesundheit', 'Persönlichkeitsentwicklung', 'Stressbewältigung']
  },
  'ernaehrung-nahrungsergaenzung': {
    name: 'Ernährung & Nahrungsergänzung', 
    keywords: ['ernährung', 'nahrung', 'lebensmittel', 'superfood', 'antioxidantien', 'entzündung', 'diet', 'kalorien'],
  ...
  'immunsystem-gesundheit': {
    ...
    keywords: ['immunsystem', 'immunität', 'antiviral', 'infektion', 'abwehr', 'widerstand', 'gesundheit'],
    pillarPost: '2023-08-14-top-lebensmittel-fuers-immunsystems',
    supportingCategories: ['Immunsystem', 'Gesundheitsvorsorge', 'Prävention']
  },
  'darm-mikrobiom': {
    name: 'Darm & Mikrobiom',
    keywords: ['darm', 'mikrobiom', 'probiotika', 'verdauung', 'bakterien', 'stuhlgang', 'magen'],
  ...
  'vitamine-mineralstoffe': {
    ...
    keywords: ['vitamin', 'mineral', 'nährstoff', 'mangel', 'defizit', 'supplementierung', 'b12', 'vitamin-d', 'vitamin-c'],
 ...
 */
const LINKING_WEIGHTS = {
  EXACT_KEYWORD_MATCH: 10,
...
function calculateContentRelationship(
  ...
  for (const keyword of sourceKeywords) {
    ...
    if (targetKeywords.some(k => k.toLowerCase() === keywordLower) || 
        targetTitle.includes(keywordLower)) {
      score += LINKING_WEIGHTS.EXACT_KEYWORD_MATCH;
      matchReasons.push(`Exact keyword match: ${keyword}`);
      suggestedAnchorText.push(keyword);
    }
    
    // Partial keyword match
    else if (targetTitle.includes(keywordLower.split(' ')[0]) || 
             targetKeywords.some(k => k.toLowerCase().includes(keywordLower.split(' ')[0]))) {
      score += LINKING_WEIGHTS.PARTIAL_KEYWORD_MATCH;
  ...
  if (sharedCategories.length > 0) {
    score += sharedCategories.length * LINKING_WEIGHTS.CATEGORY_MATCH;
  ...
  if (sharedTags.length > 0) {
    score += sharedTags.length * LINKING_WEIGHTS.TAG_MATCH;
  ...
  if (sourceCluster && targetCluster && sourceCluster === targetCluster) {
    score += LINKING_WEIGHTS.CLUSTER_MATCH;
    matchReasons.push(`Same topic cluster: ${TOPIC_CLUSTERS[sourceCluster].name}`);
  }
  
  // Cross-cluster bonus for related topics (e.g., gut health → mental health)
  if (sourceCluster && targetCluster && areRelatedClusters(sourceCluster, targetCluster)) {
    score += LINKING_WEIGHTS.CLUSTER_MATCH / 2;
    matchReasons.push(`Related topic clusters`);
  }

  // 5. Author matching (small bonus)
  if (sourcePost.data.author === targetPost.data.author) {
    score += LINKING_WEIGHTS.AUTHOR_MATCH;
  ...
  if (targetDate > oneYearAgo) {
    score += LINKING_WEIGHTS.RECENCY_BONUS;
  ...
  // Determine linking context strength
  let linkingContext: 'strong' | 'moderate' | 'weak' = 'weak';
  if (score >= 15) linkingContext = 'strong';
  else if (score >= 8) linkingContext = 'moderate';

  return {
    targetPost,
    score,
    matchReasons,
    suggestedAnchorText: [...new Set(suggestedAnchorText)], // Remove duplicates
    linkingContext
  };
}
...
function identifyTopicCluster(post: Post): keyof typeof TOPIC_CLUSTERS | null {
  ...
  const keywords = (post.data.keywords || []).map((k: string) => k.toLowerCase());
  const content = [title, ...categories, ...keywords].join(' ');

  let bestMatch: { cluster: keyof typeof TOPIC_CLUSTERS; score: number } | null = null;
...
function areRelatedClusters(
  ...
  return relatedPairs.some(([c1, c2]) => 
    (cluster1 === c1 && cluster2 === c2) || (cluster1 === c2 && cluster2 === c1)
  );
}
...
function generateSmartAnchorText(post: Post): string[] {
  ...
  if (title.toLowerCase().includes('warnzeichen') || title.toLowerCase().includes('symptome')) {
    suggestions.push('Warnzeichen erkennen', 'Symptome verstehen');
  }
  
  if (title.toLowerCase().includes('tipps') || title.toLowerCase().includes('strategien')) {
    suggestions.push('praktische Tipps', 'bewährte Strategien');
  }

  if (title.toLowerCase().includes('gesunde') || title.toLowerCase().includes('gesundheit')) {
...
function analyzeTopicClusters(posts: Post[]): TopicClusterAnalysis[] {
  ...
  for (const [clusterId, clusterInfo] of Object.entries(TOPIC_CLUSTERS)) {
    const clusterPosts = posts.filter(post => 
      identifyTopicCluster(post) === clusterId
    );

    // Find pillar post
    const pillarPost = clusterPosts.find(post => 
      post.slug === clusterInfo.pillarPost
    );

    // Separate supporting posts
    const supportingPosts = clusterPosts.filter(post => 
      post.slug !== clusterInfo.pillarPost
    );
    ...
    if (pillarPost) {
      ...
      const pillarLinks = analyzeContentRelationships(pillarPost, supportingPosts, 10);
    ...
    for (let j = i + 1; j < clusters.length; j++) {
      ...
      if (areRelatedClusters(cluster1.clusterId, cluster2.clusterId)) {
        ...
        for (const post1 of cluster1TopPosts) {
          const relationships = analyzeContentRelationships(post1, cluster2TopPosts, 2);
          crossClusterLinks.push(...relationships.filter(rel => rel.score >= 5));
        }
      }
    }
  }

  return crossClusterLinks.sort((a, b) => b.score - a.score);
}
...
function generateInternalLinkingReport(posts: Post[]): {
  ...
  const crossClusterOpportunities = findCrossClusterLinkingOpportunities(topicClusters);
  ...
  const highAuthorityPosts = posts.filter(post => 
    post.data.featured || 
    (post.data.categories && post.data.categories.length >= 3) ||
    (post.data.keywords && post.data.keywords.length >= 5)
  );

  logger.log(`Analysis complete: ${topicClusters.length} clusters, ${crossClusterOpportunities.length} cross-cluster opportunities`);
