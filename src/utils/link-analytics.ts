...
export class LinkAnalytics {
  ...
  analyzePerformance(days: number = 30): LinkPerformanceMetrics {
    ...
    const recentClicks = this.clickData.filter(
    ...
    const uniqueClicks = new Set(
      recentClicks.map(
        click => `${click.sourcePost}-${click.targetPost}-${click.sessionId}`
      )
    ).size;
    ...
    return {
      ...
      bestPerformingAnchors: Object.entries(anchorClicks)
        .map(([anchor, clicks]) => ({
          anchor,
          clicks,
  ...
  async auditInternalLinks(posts: Post[]): Promise<InternalLinkAudit> {
    ...
    audit.linkDistribution.averageLinksPerPage =
      audit.totalInternalLinks / posts.length;
    ...
    logger.log(
      `Internal link audit complete: ${audit.totalInternalLinks} total links found`
    );
    return audit;
  }
  ...
  analyzeSEOImpact(posts: Post[]): SEOLinkAnalysis {
    ...
    const totalAuthority = Object.values(analysis.pageAuthority).reduce(
      (sum, auth) => sum + auth,
      0
    );
    Object.keys(analysis.pageAuthority).forEach(slug => {
      analysis.linkJuiceDistribution[slug] =
        (analysis.pageAuthority[slug] / totalAuthority) * 100;
    });
  ...
  generateLinkingRecommendations(posts: Post[]): Array<{
    ...
    const highAuthorityPosts = posts
      .filter(
        post => post.data.featured || (post.data.categories?.length || 0) >= 3
      )
      .slice(0, 10);
    ...
    if (highAuthorityPosts.length > 0) {
      recommendations.push({
        ...
        actionItems: highAuthorityPosts.map(
          post => `Add 3-5 contextual links to "${post.data.title}"`
        ),
      });
    }
    ...
    recommendations.push({
      ...
      priority: "medium" as const,
      description:
        "Strengthen topical content clusters with better internal linking",
      actionItems: [
  ...
  private groupClicksByField(
    clicks: LinkClickData[],
    field: keyof LinkClickData
  ...
  private getTopItems(
    ...
    limit: number
  ): Array<{ page: string; clicks: number }> {
...
export function trackLinkClick(
  data: Omit<LinkClickData, "timestamp" | "sessionId">
): void {
 ...
 */
export function getLinkPerformanceMetrics(
  days: number = 30
