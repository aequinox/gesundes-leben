...
describe('isValidEmail', () => {
  ...
  describe('options handling', () => {
    it('should respect allowPlus option', () => {
      expect(isValidEmail('user+tag@example.com', { allowPlus: false })).toBe(false);
      expect(isValidEmail('user+tag@example.com', { allowPlus: true })).toBe(true);
    });

    it('should respect allowDots option', () => {
      expect(isValidEmail('user.name@example.com', { allowDots: false })).toBe(false);
      expect(isValidEmail('user.name@example.com', { allowDots: true })).toBe(true);
    });

    it('should respect minDomainLength option', () => {
      expect(isValidEmail('user@example.co', { minDomainLength: 3 })).toBe(false);
      expect(isValidEmail('user@example.com', { minDomainLength: 3 })).toBe(true);
    });
...
describe('validateEmails', () => {
  it('should validate multiple emails', () => {
    ...
    expect(result.results[0]).toEqual({ email: 'valid@example.com', isValid: true });
    expect(result.results[1]).toEqual({ email: 'invalid-email', isValid: false });
  });
