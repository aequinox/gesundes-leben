...
describe('Design System Utilities', () => {
  ...
  describe('combineClasses', () => {
    it('should combine multiple class sets', () => {
      ...
      expect(combineClasses(set1, set2)).toEqual(['class1', 'class2', 'class3', 'class4']);
    });
...
describe('Design System Styles', () => {
  describe('focusStyles', () => {
    ...
    it('should provide OKLCH outline styles', () => {
      expect(focusStyles.oklchOutline).toContain('focus-visible:[outline:2px_solid_oklch(var(--color-accent))]');
    });
  });

  describe('shadowStyles', () => {
    it('should provide elevation shadow styles', () => {
      expect(shadowStyles.elevation.low).toContain('shadow-[var(--shadow-elevation-low)]');
      expect(shadowStyles.elevation.medium).toContain('shadow-[var(--shadow-elevation-medium)]');
      expect(shadowStyles.elevation.high).toContain('shadow-[var(--shadow-elevation-high)]');
    });

    it('should provide colored shadow styles', () => {
      expect(shadowStyles.colored.pro).toContain('shadow-[var(--shadow-pro)]');
      expect(shadowStyles.colored.contra).toContain('shadow-[var(--shadow-contra)]');
      expect(shadowStyles.colored.questionTime).toContain('shadow-[var(--shadow-question-time)]');
    });

    it('should provide hover transition styles', () => {
      expect(shadowStyles.hoverTransitions.elevate).toContain('hover:shadow-[var(--shadow-elevation-medium)]');
      expect(shadowStyles.hoverTransitions.elevate).toContain('transition-shadow');
    });
  });
  ...
  describe('animationStyles', () => {
    ...
    it('should provide motion reduction styles', () => {
      expect(animationStyles.motionSafe).toContain('motion-reduce:transform-none');
      expect(animationStyles.motionSafe).toContain('motion-reduce:transition-none');
    });
  });
});
